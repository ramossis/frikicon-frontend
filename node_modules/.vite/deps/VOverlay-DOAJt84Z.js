import { $n as toRef, G as createBaseVNode, Gn as reactive, Hn as onScopeDispose, It as onMounted, Kn as readonly, L as Teleport, Mt as onBeforeUnmount, N as Fragment, Nn as effectScope, Ot as mergeProps, Qn as toRaw, St as inject, W as computed, Wt as provide, Xn as shallowRef, _n as watch, gn as warn, kt as nextTick, qn as ref, r as Transition, tr as toValue, tt as createVNode, vn as watchEffect, w as vShow, wn as withDirectives } from "./vue.runtime.esm-bundler-BQrNAWUe.js";
import { $ as templateRef, A as getActiveElement, G as omit, H as matchesSelector, R as isOn, S as destructComputed, St as isString, _t as isFunction, bt as isNumber, d as propsFactory, f as CircularBuffer, jt as consoleError, k as focusableChildren, l as getCurrentInstance, n as genericComponent, nt as IN_BROWSER, v as clamp, w as eventName, x as defer, y as convertToUnit, yt as isNullOrUndefined } from "./defineComponent-DrE4ifQT.js";
import { a as parseAnchor, i as getAxis, n as flipCorner, r as flipSide, t as flipAlign } from "./anchor-e4czR62_.js";
import { i as getTargetBox, n as getElementBox, r as getOverflow, t as Box } from "./box-BkJif17a.js";
import { v as animate, y as nullifyTransforms } from "./transitions-BvVY0ByH.js";
import { n as makeComponentProps, t as useRender } from "./useRender-0m5V27n9.js";
import { t as deepEqual } from "./deepEqual-BGv70i0V.js";
import "./easing-DZ7EhLCz.js";
import { a as hasScrollbar, i as getScrollParents, n as useLazy, r as getScrollParent, t as makeLazyProps } from "./lazy-BtCdlfQq.js";
...
import { i as useRouter, n as useBackButton } from "./router-BTTg3a4c.js";
...
import "/home/ramos/Escritorio/proyecto_frikivon/friki_con/node_modules/vuetify/lib/components/VOverlay/VOverlay.css";
...
function bindProps(el, props) {
	Object.keys(props).forEach((k) => {
		if (isOn(k)) {
			...
			if (isNullOrUndefined(props[k])) handler?.forEach((v) => {
				const [n, fn] = v;
				if (n === name) {
		...
		} else if (isNullOrUndefined(props[k])) el.removeAttribute(k);
...
function useLocationStrategies(props, data) {
	...
	if (IN_BROWSER) useToggleScope(() => !!(data.isActive.value && props.locationStrategy), (reset) => {
		...
		if (isFunction(props.locationStrategy)) updateLocation.value = props.locationStrategy(data, props, contentStyles)?.updateLocation;
...
function getIntrinsicSize(el, isRtl) {
	...
	if (isRtl) contentBox.x += offset("right");
	else contentBox.x -= offset("left");
...
function connectedLocationStrategy(data, props, contentStyles) {
	...
	const [getMinWidth, getMinHeight, getMaxWidth, getMaxHeight] = [
		...
		return () => {
			...
			if (isNumber(raw) || /^-?[\d.]+(?:px)?$/.test(raw.trim())) return parseFloat(raw);
			if (raw.endsWith("%")) {
...
function useScrollStrategies(props, data) {
	...
	watchEffect(async () => {
		...
		scope.active && scope.run(() => {
			if (isFunction(props.scrollStrategy)) props.scrollStrategy(data, props, scope);
...
function useActivator(props, { isActive, isTop, contentEl, isSubmenu = false }) {
	...
	watch(isActive, (v) => {
		...
		if (!delayCallbackFiredOpen) openedByHover.value = false;
		delayCallbackFiredOpen = false;
	});
	...
	watch(isTop, (val) => {
		if (val && shouldCloseOnLeave() && (props.openOnHover && !isHovered && (!openOnFocus.value || !isFocused) || openOnFocus.value && !isFocused && (!props.openOnHover || !isHovered)) && !contentEl.value?.contains(getActiveElement())) runCloseDelay();
	});
	watch(isActive, (val) => {
		if (!val) setTimeout(() => {
...
function getTarget(selector, vm) {
	...
	if (selector === "parent") {
		...
		target = el;
	} else if (isString(selector)) target = document.querySelector(selector);
...
function checkEvent(e, el, binding, ignoreActive = false) {
	...
	const elements = ((isFunction(value) ? void 0 : value.include) || (() => []))();
	elements.push(el);
	return !elements.some((el) => el?.contains(e.target));
}
function checkIsActive(e, binding) {
	const value = binding.value;
	return ((isFunction(value) ? void 0 : value.closeConditional) || defaultConditional)(e);
}
...
var overlayActivators = /* @__PURE__ */ new WeakMap();
...
var VOverlay = genericComponent()({
	...
	setup(props, { slots, attrs, emit }) {
		...
		const { activatorEl, activatorRef, target, targetEl, targetRef, activatorEvents, contentEvents, scrimEvents, openedByHover } = useActivator(props, {
			isActive,
			isTop: localTop,
			contentEl,
		...
		const menu = inject(VMenuSymbol, null);
		if (vm.parent?.type?.name !== "VMenu") provide(VMenuSymbol, null);
		...
		watch(isActive, (val) => {
			if (val) {
				...
				if (contentEl.value?._clickOutside) contentEl.value._clickOutside.lastMousedownWasOutside = false;
			} else {
		...
		function onKeydown(e) {
			if (e.key === "Escape" && globalTop.value) {
				if (!contentEl.value?.contains(getActiveElement())) emit("keydown", e);
				if (!props.persistent) {
					isActive.value = false;
					if (contentEl.value?.contains(getActiveElement())) activatorEl.value?.focus();
				} else animateClick();
			}
		}
		...
		useRender(() => createBaseVNode(Fragment, null, [slots.activator?.({
			...
			include: () => {
				...
				return [activatorEl.value, ...Array.from(document.querySelectorAll(".v-overlay__content")).filter(ownsFocus)];
			}
		}]])] })])] })]));
		return {
			activatorEl,
