...
import { toAgentToolName } from "../utils/agent-tool-name.mjs";
...
function createLiveRpcClientMode(options) {
	...
	*/
	...
	async function requestAuthCode(options = {}) {
...
*/
function registerWebMcpTools(clientRpc, options = {}) {
...
function resolveSafety(def, agent) {
...
async function executeRpcTool(def, context, args) {
	try {
		const positional = coerceAgentPositionalArgs(args, def.args, "wrap");
		return { content: [{
			type: "text",
...
async function getDevframeRpcClient(options = {}) {
	...
	const disposeWebMcp = options.webmcp === false ? void 0 : registerWebMcpTools(clientRpc);
...
//#endregion
export { DevframeConnectionError, authenticateWithUrlOtp, connectDevframe, consumeOtpFromUrl, createClientSettings, createRpcStreamingClientHost, createScopedClientContext, getDevframeConnection, getDevframeRpcClient, isCallableStatus, readOtpFromUrl, registerDevframeViewerOrigin, registerWebMcpTools, resolveClientTransport, resolveSseUrl, resolveWebMcpModelContext, resolveWsUrl, setupDevframeConnection };
