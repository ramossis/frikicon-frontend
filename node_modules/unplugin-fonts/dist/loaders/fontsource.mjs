//#region src/loaders/fontsource.ts
function fontsourceImports(options) {
	...
	for (const family of families) {
		...
		if (typeof family === "string") {
			...
			if (name.endsWith("-variable")) source.push(`@fontsource-variable/${name.replace("-variable", "")}/index.css`);
			else source.push(`@fontsource/${name}/index.css`);
			continue;
		}
		const name = family.name.toLowerCase().replace(/ /g, "-").replace("-variable", "");
		...
		if ("variable" in family) {
			...
			if (activeAxes.length === 2 && family.variable.wght) {
				const selected = activeAxes.find((axis) => axis !== "wght")?.toLowerCase() ?? "wght";
				if (isItal) {
		...
		if ("weights" in family) {
			...
			for (const weight of family.weights) if (family.styles) for (const style of family.styles) if (style === "normal") source.push(`@fontsource/${name}/${subsetPrefix}${weight}.css`);
			else source.push(`@fontsource/${name}/${subsetPrefix}${weight}-${style}.css`);
			else source.push(`@fontsource/${name}/${subsetPrefix}${weight}.css`);
		}
	}
	return source;
}
...
//#endregion
export { fontsourceImports, fontsourceVirtualModule };
