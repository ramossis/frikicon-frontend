...
async function parseApply({ code, uno, applyVariable }, node, childNode) {
	...
	const utils = (await Promise.all(classNames.map((i) => uno.parseToken(i, "-")))).filter(notNull).flat().sort((a, b) => a[0] - b[0]).sort((a, b) => (a[3] ? uno.getParentOrder(a[3]) ?? 0 : 0) - (b[3] ? uno.getParentOrder(b[3]) ?? 0 : 0)).reduce((acc, item) => {
...
function handleScreen({ code, uno }, node) {
	...
	const generateMediaQuery = (breakpointName, prefix) => {
		...
		if (prefix) {
			if (prefix === "lt") return `@media (max-width: ${calcMaxWidthBySize(size)})`;
...
function resolveApplyVariables(options) {
	...
	if (applyVariable === void 0) applyVariable = [
		"--at-apply",
		"--uno-apply",
		"--uno"
	];
