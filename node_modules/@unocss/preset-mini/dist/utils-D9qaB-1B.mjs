...
function colorResolver(property, varName, key, shouldPass) {
	return ([, body], { theme, generator }) => {
		...
		const css = {};
		if (cssColor) {
			if (alpha != null) css[property] = colorToString(cssColor, alpha) + rawColorComment;
			else {
			...
			if (alpha != null) css[property] = colorToString(color, alpha) + rawColorComment;
			else {
				const opacityVar = `--un-${varName}-opacity`;
...
function isSize(str) {
	if (str[0] === "[" && str.endsWith("]")) str = str.slice(1, -1);
