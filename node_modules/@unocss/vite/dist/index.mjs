	...
	return [
		{
			...
			async configResolved(config) {
				...
				for (const env of Object.values(config.environments ?? {})) if (env?.build?.outDir) distDirs.push(resolve(config.root, env.build.outDir));
...
function toViteVirtualId(id) {
	return id.startsWith(VITE_VIRTUAL_ID_PREFIX) ? id : `${VITE_VIRTUAL_ID_PREFIX}${id}`;
}
function toViteClientPath(id) {
	return id.startsWith(VITE_VIRTUAL_ID_PREFIX) ? `${VITE_CLIENT_ID_PREFIX}${id.replace(VITE_VIRTUAL_ID_PREFIX, VITE_NULL_BYTE_PLACEHOLDER)}` : id;
}
//#endregion
...
function PerModuleModePlugin(ctx) {
	...
	return [{
		...
		async resolveId(id) {
			const entry = await resolveId(ctx, id);
			if (entry) return toViteVirtualId(entry);
		},
		async load(id) {
		...
		resolveId(id) {
			if (id.startsWith(VIRTUAL_PREFIX)) return toViteVirtualId(id);
			return id.startsWith(VIRTUAL_ID_PREFIX) ? id : null;
		},
		load(id) {
			if (!id.startsWith(VIRTUAL_ID_PREFIX)) return null;
			const hash = getPath(id).slice(VIRTUAL_ID_PREFIX.length, -4);
